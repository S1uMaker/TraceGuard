...
v0.1.0 基线已通过 Ubuntu 首轮验收；当前 v0.4.0 源码已于 2026-09-22 重新构建并完成完整验收和 root 包管理器专项场景。详细 verifier 失败日志没有通过故意篡改 BPF 对象触发，其余本轮覆盖见 [最新验证报告](validation/2026-09-22/REPORT.md)。
...
`list-rules` 是只读取配置的独立命令，先严格校验，再按配置顺序显示所有规则的条件和启用状态。JSON 输出直接使用现有规则结构，文本将任意配置字符串转义。该入口不创建规则运行实例，不访问 Docker、BPF 或输出日志；它参考 Falco 的规则清单能力，帮助解释当前配置。

普通回放和 `replay --dry-run` 共用 `replayWithMode` 的读取、解析、事件验证及规则处理；保留原三参数 `replay` 入口。正常回放仍要求独立输出目录、检查输入输出 inode 并持有目录锁。预览以显式 `dryRun` 跳过 writer 初始化及两种持久化写入，不通过空 writer 假装写入成功；CLI 拒绝同时传入 `--output`，也不检查配置输出路径。

预览保留 `mode=replay-dry-run`，`saved`／`alerts` 恒为 0，分别使用 `preview_alerts`／`preview_alerts_by_rule` 记录尝试显示的告警。例外计数在预览评估后增加，来源未知及主机过滤按原逻辑累计。所有字段仍仅代表本次进程；输入坏行或 stdout 写入失败会结束处理并报告行号，已经输出的部分不构成完整成功结果。详细字段契约见 [配置说明](../configs/README.md)。

加载诊断只在 `ebpf.NewCollection` 失败处用 `errors.As` 取出 `*ebpf.VerifierError`，直接格式化该错误获取库提供的 verifier 日志，同时保留 `%w` 原错误链。成功加载、map／探针结构和 reader 生命周期不变；没有对应类型的错误继续用原提示，避免把所有加载错误误判成权限问题。参考链接见 [开源说明](OPEN_SOURCE.md)。

## v0.4.0 的具体检测场景

`docker-root-package-manager` 把现有四类条件组合成一个明确场景：成功 exec、已确认 Docker 来源、UID 0，以及十个操作系统包管理器之一。实现只增加默认配置规则，不增加引擎分支；因此实时与回放、规则例外、证据生成及分类统计继续走相同路径。

这个告警用于提示可能的运行时容器漂移。判断链可以逐项解释，但结论必须停在“包管理器被启动”：现有事件不含 argv、父进程、网络或文件变更，无法区分 `apt-get --version` 与 `apt-get install`，也无法确认安装结果。将 UID 限定为 0 可缩小默认噪声，同时会漏掉非 root 包管理器执行；Docker 镜像构建、获准维护和启动时安装依赖仍可能产生正常告警。

独立合成输入覆盖一个命中项及非 root、普通进程、宿主机三个对照，不改变 v0.1.0 已验证的原三行回放夹具。2026-09-22 的不落盘回放只命中 root 正例，真实容器采集也只对 UID 0 的 `apt-get` 产生新规则告警。完整分析流程和调优边界见 [场景说明](SCENARIOS.md)。
...
这段表述对应当前 v0.4.0 已实现并在上述环境重新验证的主流程与专项场景。尚未进行性能或压力测试，
